...
import { ArrowLeft, Save, Image, IndianRupee, Upload, X, Loader2, Info, Package } from 'lucide-react';
...
export const AddProductPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      showToast('Failed to create product in database.', 'error');
    } finally {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Create a new product in your MerchHQ store.</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Wireless Active Noise Cancelling Headphones"
        ...
        <div className="seller-card p-6 space-y-5">
          ...
          <div className="p-4 rounded-xl bg-blue-50/60 border border-blue-100 flex flex-wrap items-center justify-between gap-4 text-xs">
            <div className="flex items-center gap-2 text-blue-900 font-medium">
              ...
              <span>Base {formatPrice(basePrice)} − Discount {formatPrice(discountAmt)} + GST ({formData.gstPercentage}%) {formatPrice(gstAmt)}</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] text-slate-500 font-bold uppercase block">Final Selling Price</span>
        ...
        <div className="seller-card p-6 space-y-5">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-3 pointer-events-none">
              ...
              <div>
                <p className="text-sm font-bold text-slate-800">Click to upload or drag & drop images here</p>
                <p className="text-xs text-slate-400 mt-1">PNG, JPG, WEBP supported · Multiple files allowed</p>
              </div>
              <p className="text-[10px] text-slate-400 font-mono">Images → FastAPI → Cloudinary → MongoDB</p>
            </div>
          </div>
              ...
              {images.map((img, index) => (
                <div key={index} className="relative group aspect-square rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
                  ...
                  {!img.uploading && !img.error && img.url && (
                    <div className="absolute top-1.5 left-1.5 bg-emerald-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">
                      ✓ Uploaded
                    </div>
                  )}
